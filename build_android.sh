...
=======
echo ==========
# Set default value for Build_Skip
Build_Skip=${Build_Skip:-false}#!/bin/sh -e
...
GotoEND=false
Build_Skip=false
if [ -d "${RootDir}/target/aarch64-linux-android/$MODE" ]; then
    echo "update skip"
    Build_Skip=true
else
...
if [ "$GotoEND" = "false" ]; then
    ...
    fvm flutter build apk --target-platform android-arm64 --${MODE} --obfuscate --split-debug-info ./split-debug-info
    # fvm flutter build apk --split-per-abi --target-platform android-arm64,android-arm --${MODE} --obfuscate --split-debug-info ./split-debug-info
    # fvm flutter build appbundle --target-platform  android-arm64,android-arm --${MODE} --obfuscate --split-debug-info ./split-debug-info
    popd
