...
class PlatformFFI {
  ...
  FuckDeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(FuckDeskImpl FuckDeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = FuckDeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
