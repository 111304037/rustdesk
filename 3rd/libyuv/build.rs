...
fn main() -> anyhow::Result<()> {
    ...
    println!("library_name: {}", library_name(true));
    ...
    if !is_exsit(&join(&output, &library_name(true))?) {
        ...
        if download_result.is_err() {
            ...
            fs::copy(source_path, join(&output, &library_name(true))?)
                .expect("Failed to copy local libyuv.a file");
            println!("cargo:warning=Successfully copied local libyuv.a file");
        }
    }
